...
class phpExcelExport extends Controller{
	...
	public function departmentNegative($dpt, $from, $to)
	{
		...
		$this->setReportWithDepartmentNegative($header, $report, $bold, "A", "", "D", "[ DPT : " . $dpt . " - ".$report[0]['DptName']." ] - [ 
			".$from." - ".$to." ]");
	...
	private function setReportWithDepartmentNegative($header, $report, $bold, $upc_col = "A", $unit_price_col = "", $itemDescription = "D", $subtitle)
	{
		$j = 4;
		$countItems = 0;
		$lastKey = $this->getLastArrayKey($header);
		...
		for ($i=0; $i<count($report); $i++)
		{
			if(!in_array($report[$i]['UPC'], $allUPCS)){
            	$allUPCS = array_merge(array($report[$i]['UPC']), $allUPCS);
            	if($report[$i]['onhand'] < 0 && $report[$i]["SctNo"] != 184)
			{
				$countItems++;
				if($increment == 0 || $condition != $report[$i]["SctNo"])
